...
export function imageToCanvas(img: HTMLImageElement, size: ImageSize = { width: img.naturalWidth, height: img.naturalWidth }) {
  ...
  !isVoid(ctx) && ctx.drawImage(img, 0, 0, cvs.width, cvs.height)
  return cvs
}
...
*/
export function imageResize(img: HTMLImageElement, options: ImageOptions) {
  const resizeImg = new Image()
  const cvs = imageToCanvas(img, options)
  const type = options.type ?? imageGetType(img.src)
  const quality = options.quality ?? 1
  resizeImg.src = cvs.toDataURL(type, mathBetween(quality, 0, 1))
  return resizeImg
}
...
*/
export function imageMini(img: HTMLImageElement, quality: number) {
  const resizeImg = new Image()
  const cvs = imageToCanvas(img, { width: img.naturalWidth, height: img.naturalHeight })
  const type = imageGetType(img.src)
  resizeImg.src = cvs.toDataURL(type, mathBetween(quality, 0, 1))
  return resizeImg
}