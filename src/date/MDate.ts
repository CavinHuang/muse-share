...
import { isArray, isDate, isObject, isString } from '../is'
import type { Lang, LangConfigItem } from './lang'
...
export class MDate {
  ...
  format(pattern = 'YYYY-MM-DD HH:mm:ss') {
    ...
    const _ = that._date
    const $ = that.toObject()
    const match = _.toTimeString().match(/GMT([\+\-])(\d{2})(\d{2})/)
    if (match) {
      const map: { [k in string]: string | number } = {
        YYYY: `${$.year}`,
      ...
      const langMap = Object.assign({}, this._langMap)
  ...
  startOf(unit: (typeof period)[number] | 'timestamp' | 'week') {
    const $ = this.toObject()
    let that = null
    const index = period.indexOf(unit as unknown as any) + 1
    const dateSet = this.toArray()
    const initSet = initTime.slice(index)
  ...
  diffWith(that: MDate | string | Date | number, unit: (typeof period)[number]) {
    ...
    const timestamp = this.getTime() - that.getTime()
    let value = 0
    if (unit) {
      if ((diffMap as any)[unit]) {
        ...
        const this_month = 12 * (this.get('year') - 1) + this.get('month')
        const that_month = 12 * (that.get('year') - 1) + that.get('month')
      ...
      const hash: { [k in (typeof period)[number]]?: number | boolean } = {}
      period.forEach((unit) => {
...
export function mdate(...args: any[]) {
  ...
  if (args.length) {
    if (isDate(args[0])) {
      ...
      if (args.length >= 2 && !Number.isNaN(args[0]) && args[0] < 100)
